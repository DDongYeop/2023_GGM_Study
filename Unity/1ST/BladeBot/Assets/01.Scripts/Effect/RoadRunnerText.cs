...
public class TypeChar
{
    ...
    public TypeChar(int start, Vector3[] vertices, Color32[] colors, Color startColor, Color endColor, float delayTime, float effectTime = 0.5f, float offset = 15f)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            Vector3 point = vertices[start + i]; //이걸 하는 이유는 벡터3을 복사하기 위함
            _originPos[i] = point;
            ...
            {
                vertices[start + i] = point + new Vector3(offset + 0.25f, 0, 0); //우측으로 오프셋 + 0.25f간 곳
            }
            colors[start + i].a = 0; //투명도만 0으로 
        }
    ...
    public void UpdateMesh(Vector3[] vertices, Color32[] colors)
    {
        ...
        if (_currentTime < _delayTime || IsComplete)
            return; //딜레이 타임 안 지났거나 완료된 이펙트라면 아무것도 안 함
