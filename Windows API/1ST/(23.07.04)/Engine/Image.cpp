...
void Image::DrawAnimation(HDC hdc, int x, int y, shared_ptr<Animation> ani)
{
	if (ani == nullptr)
		return;

	int frameX = ani->GetFramePos().x;
	int frameY = ani->GetFramePos().y;
	int frameW = ani->GetFrameWidth();
	int frameH = ani->GetFrameHeight();
	
	DrawBitmap(hdc, x, y, frameW, frameH, frameX, frameY, frameW, frameH);
}
